...
for (page in content) getPage(page);
...
chrome.tabs.query({currentWindow: true, active: true},function (tab) {
    ...
    setupCss();
    loadData(); // This sets up everything.
...
{
    if ( tagText == "" ) return; // We are getting blank ones due to double ups of events.  This is the easy fix.
    bg.addTag(tagText);
    refreshTags();
    refreshSubmissionData();
}
...
function fromTagged()
{
    ...
    if ( doingTutorial() ) {
        bg.tags = new Array(); // Clear the tags, tutorial only has one and it needs locking down.
    }
...
function loadData()
{
    ...
    if (bg.submittingRebuttal) {
        displaySubmissionForm();
    } else if (recordedClick != null) { // This means we are on a rebuttal we clicked through to.
...
//    $('#voteUpDiv').html(bg.voteUpDiv);
    if ( ! bg.submittingRebuttal) { // Don't show current rebuttals if adding, too messy.
        waitCount = 0; // Needs to be set for following call which is recursive
...
{ // Recursively sleep until the rebuttal data is ready.
    setTimeout(function()
    {
        if (bg.rebuttals[tabId] == null) { // not yet ready.
            waitCount++;
            if (waitCount < 50) {
                handleDelayOnLoadOfRebuttals(); // Recurse
                return;
            }
            if (!bg.canonical_urls[tabId]) {
